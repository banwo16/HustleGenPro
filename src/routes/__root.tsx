...
export const Route = createRootRoute({
  head: () => ({
    meta: [
      ...
      {
        property: 'og:title',
        content: 'HustleGenPro — AI Pitch Coach for Beginner Freelancers',
      },
      {
      ...
      { property: 'og:site_name', content: 'HustleGenPro' },
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              '@graph': [
                { '@type': 'WebSite', name: 'HustleGenPro', url: '/' },
                { '@type': 'Organization', name: 'HustleGenPro', url: '/', sameAs: [] },
              ],
            }),
          }}
        />
        ...
        <QueryClientProvider client={queryClient}>
          <TooltipProvider delayDuration={0}>
            ...
            */}
            {children}
            <BackToTop />
            {/* Login/signup modal — browser-only (Supabase auth calls), so it's
                wrapped the same way Blink's auth state used to be. */}
            <BlinkClientBoundary>
              <AuthModal />
            </BlinkClientBoundary>
          </TooltipProvider>
