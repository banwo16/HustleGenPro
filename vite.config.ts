...
export default defineConfig({
  ...
  resolve: {
    ...
    // framer-motion + R3F peers must share one React instance or hooks
  ...
  build: {
    ...
    // .vite-out/client/* -> dist/, which is what gets deployed as the Netlify
    // publish directory (static site, no SSR server needed at runtime).
    // Building here instead of directly into dist/ avoids EACCES errors if a
    // prior build already left a read-only _redirects file in dist/.
    outDir: '.vite-out',
    emptyOutDir: true,
  },
});
